...
INTO TABLE aws_billing_detailed_line_items_resources_tags_AWSMarketplace
FIELDS TERMINATED BY ','
ENCLOSED BY '\"'
IGNORE 1 LINES
(invoice_i_d,payer_account_id,linked_account_id,record_type,record_id,product_name,rate_id,subscription_id,pricing_plan_id,usage_type,operation,availability_zone,reserved_instance,item_description,usage_start_date,usage_end_date,usage_quantity,rate,cost,UnBlendedRate,UnBlendedCost,resource_id) SET month_ = $DATE, account_ = $ACCOUNTID, imported_date = $IMPORTDATE ; " -e "SELECT ROW_COUNT();"`
